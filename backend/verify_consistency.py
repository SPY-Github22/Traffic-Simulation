...
def test_logical_consistency():
    ...
    for edge, score in scores_base.items():
        assert 1.0 <= score <= 10.0, f"Congestion score {score} for edge {edge} is out of bounds [1.0, 10.0]"
    ...
    for edge, score in scores_mitigated_direct.items():
        if edge in barricades_direct:
            assert score == 0.0, f"Barricaded edge {edge} must have 0.0 congestion score"
        else:
            assert 1.0 <= score <= 10.0, f"Congestion score {score} for edge {edge} is out of bounds [1.0, 10.0]"
